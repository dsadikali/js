...
function create_pagination(pagecnt){
	...
	totalparent = $('.news-container').size();
	totalbox=$('.newsbox ').size();
	$('.articleNums').html(totalbox+' Artikel Gefunden');
	pagecnt = Math.floor( totalparent/noofpage );
	pagerem = $('.news-container').size()%noofpage ;
...
function create_pages(cnt){
	...
	$('.newsbox:first-child').addClass('first');
	...
	$('.news-container').eq(goon).addClass('active');
...
$(document).ready(function(){
	$('.newsbox :last-child').addClass('last');
	create_pagination(4);
